...
Regenerate the `MindElixir` class members:

The option and method declarations of the `MindElixir` class (the `// #region GENERATED` block in `src/index.ts`) are generated by `gen-members.js` from the compiled declarations in `dist/types`, so the published `.d.ts` and the API docs show fully expanded signatures. After changing the mixed-in methods (`src/methods.ts`) or the `Options` interface (`src/types/index.ts`), run:

```
pnpm tsc         # emit fresh dist/types
pnpm gen:members # rewrite the generated block in src/index.ts
```

The compile-time guards at the bottom of `src/index.ts` fail `tsc` whenever the block drifts out of sync, so a forgotten regeneration can't slip through.
