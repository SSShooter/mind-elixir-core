...
- The outliner ships as its own bundle: `import { Outliner } from 'mind-elixir/outliner'` loads the outline without the map, and `import 'mind-elixir/outliner/style.css'` brings the stylesheet it needs. The main entry keeps re-exporting it, so `import { Outliner } from 'mind-elixir'` is unchanged and `mind-elixir/style.css` still styles both views. The standalone bundle is self-contained — it bundles the few tree helpers it uses and shares no module state with the map — and a bound outline behaves identically either way: it renders `mei.nodeData` by reference and syncs on `mei.historyStack`.
...
- A collapsed outliner row no longer shows its `…` item menu while the row is neither hovered nor focused. Folding reveals the chevron so the hidden subtree stays discoverable, and the `…` was dragged along by that reveal even though its surface is painted on hover/focus only — which left a bare, unstyled glyph beside the blue chevron. The chevron still answers to the fold state; `…` keeps the same hover / focus / open signals as every other row.
- An item's controls are now revealed **and painted** by focus as well as hover, so the first tap on a node shows the `…` menu on a touch screen. The tap used to flash the buttons once and leave them hidden: it set `:hover`, and the markup swap that focusing performs (`handleFocusIn`) dropped that hover again — only a second tap, with the topic already focused and nothing left to replace, kept the group up. The same swap also left the two button surfaces (`…` and the collapse chevron) unpainted on that first tap, because their background was a separate `:hover`-only rule — visible but flat until the second tap. `:focus-within` and `:hover` now share all three rules, and the reveal stays scoped to the item that is focused or open.
- The outliner's collapse chevron answers with a pointer cursor. Its hand cursor came from Tailwind's preflight in react-outliner, which this library does not ship, so the reset that neutralises the UA button styling has to restore the cursor as well — the `…` button beside it declared its own and was the only one that felt clickable.
