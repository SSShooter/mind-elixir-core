...
export default function linkDiv(primaryNode) {
  ...
  for (let i = 0; i < primaryNodeList.length; i++) {
    ...
    if (el.className === "lhs") {
      ...
      if (this.primaryLinkStyle === 2) {
        ...
        path += `M ${10000} ${10000} C 10000 10000 ${10000 + 2 * primaryNodeHorizontalGap * 0.03} ${y2} ${x2} ${y2}`;
      }
  ...
  for (let prop in this.linkData) {
    ...
    if (typeof link.from === "string") this.createLink(findEle(link.from), findEle(link.to), true, link);
    else this.createLink(findEle(link.from.nodeObj.id), findEle(link.to.nodeObj.id), true, link);
  }
}
