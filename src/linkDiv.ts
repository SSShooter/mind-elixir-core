...
const linkDiv = function (this: MindElixirInstance, mainNode?: Wrapper) {
  ...
  // pin center
  this.map.style.top = `${(this.container.offsetHeight - this.nodes.offsetHeight) / 2}px`
  if (this.alignment === 'root') {
    this.map.style.left = `${this.container.offsetWidth / 2 - pL - pW / 2}px`
  } else {
    this.map.style.left = `${this.container.offsetWidth / 2 - nw / 2}px`
  }
