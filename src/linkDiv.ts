...
const linkDiv = function (this: MindElixirInstance, mainNode?: Wrapper) {
  ...
  // pin center
  this.nodes.style.top = `${(this.container.offsetHeight - this.nodes.offsetHeight) / 2}px`
  if (this.alignment === 'root') {
    this.nodes.style.left = `${this.container.offsetWidth / 2 - pL - pW / 2}px`
  } else {
    this.nodes.style.left = `${this.container.offsetWidth / 2 - nw / 2}px`
  }
